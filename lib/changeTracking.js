 ...
 * @param {string} detailedMessage - Optional detailed message to override default formatting
 * @returns {string} Formatted change log entry
 */
function formatChangeLog(changeType, oldValue, newValue, user, detailedMessage = null) {
    ...
    // If a detailed message is provided, use it instead of the default formatting
    if (detailedMessage) {
        return `[${timestamp}] ${user || 'System'} - ${changeType}: ${detailedMessage}`;
    }
